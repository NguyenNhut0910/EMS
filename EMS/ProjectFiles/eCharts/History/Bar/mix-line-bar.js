...
function processData(data, mode) {
  ...
  // Tập hợp tất cả device_id có mặt
  // const allDeviceIds = new Set();
  // for (const entry of energyMap.values()) {
  //   Object.keys(entry).forEach(key => {
  //     if (key.startsWith("device_")) allDeviceIds.add(key);
  //   });
  // }
...
function fetchAndRender() {
  ...
  API_URL.searchParams.append("timestamp", `gte.${startTime}`);
  API_URL.searchParams.append("timestamp", `lte.${endTime}`);
